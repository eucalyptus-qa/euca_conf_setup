...
sub get_clc_ip{
	...
	$ENV{'CLC_IP'} = "";
	foreach my $l (@temp){
        	if( $l =~ /(192\.168\.\d+\.\d+)/ && $ENV{'CLC_IP'} eq "" ){
                	$ENV{'CLC_IP'} = $1;
        	};
	};
        return 0;
...
sub post_ops_mod_euca_conf{
	...
	if( $DISTRO eq "UBUNTU" ){
		...
		my_sed( "VNET_PUBINTERFACE=\".*\"", "VNET_PUBINTERFACE=\"$ENV{'PUB_INTERFACE'}\"", $euca_conf );
		my_sed( "VNET_PRIVINTERFACE=\".*\"", "VNET_PRIVINTERFACE=\"$ENV{'PRIV_INTERFACE'}\"", $euca_conf );

		if( does_It_Have($ROLL, "NC") ){
			if( $ENV{'BUILD_NETWORK'} eq "MANAGED" ){
				my_sed( "VNET_BRIDGE=\".*\"", "VNET_BRIDGE=\"$ENV{'PUB_INTERFACE'}\"", $euca_conf );
			}elsif( $ENV{'BUILD_NETWORK'} eq "SYSTEM" || $ENV{'BUILD_NETWORK'} eq "MANAGED-NOVLAN" || $ENV{'BUILD_NETWORK'} eq "STATIC"  ){
				my_sed( "VNET_BRIDGE=\".*\"", "VNET_BRIDGE=\"br0\"", $euca_conf );
			}else{
				# ERROR !!!
			};
		};
		...
		if( does_It_Have($ROLL, "CC") ){
			if( $VERSION ne "PRECISE" ){		### ADDED 031212
				my_sed( "VNET_DHCPDAEMON=\".*\"", "VNET_DHCPDAEMON=\"\/usr\/sbin\/dhcpd3\"", $euca_conf );
...
#			my_sed( "VNET_BRIDGE=\"xenbr0\"", "VNET_BRIDGE=\"br0\"", $euca_conf );
		...
		if( does_It_Have($ROLL, "CC") ){
			### changed back ... 091010
			my_sed( "VNET_DHCPDAEMON=\".*\"", "VNET_DHCPDAEMON=\"\/usr\/sbin\/dhcpd\"", $euca_conf );
...
#			my_sed( "VNET_BRIDGE=\"xenbr0\"", "VNET_BRIDGE=\"eth0\"", $euca_conf );
                ...
                my_sed( "VNET_INTERFACE=\".*\"", "VNET_INTERFACE=\"br0\"", $euca_conf );
		...
		###	CENTOS 6 addtion 030112
		if( $VERSION =~ /^6\./ &&  does_It_Have($ROLL, "NC") ){
			if( $ENV{'BUILD_NETWORK'} eq "MANAGED" ){
				my_sed( "VNET_BRIDGE=\".*\"", "VNET_BRIDGE=\"$ENV{'PUB_INTERFACE'}\"", $euca_conf );
			}elsif( $ENV{'BUILD_NETWORK'} eq "SYSTEM" || $ENV{'BUILD_NETWORK'} eq "MANAGED-NOVLAN" || $ENV{'BUILD_NETWORK'} eq "STATIC"  ){
				my_sed( "VNET_BRIDGE=\".*\"", "VNET_BRIDGE=\"br0\"", $euca_conf );
...
#			my_sed( "VNET_BRIDGE=\"xenbr0\"", "VNET_BRIDGE=\"$ENV{'PUB_INTERFACE'}\"", $euca_conf );
		...
		### TEMP. ADDED 102711	dhcp-41 issue
		if( ($DISTRO eq "RHEL" || $DISTRO eq "CENTOS")  && $VERSION =~ /^6\./ &&  does_It_Have($ROLL, "CC") ){
			my_sed( "VNET_DHCPDAEMON=\".*\"", "VNET_DHCPDAEMON=\"\/usr\/sbin\/dhcpd41\"", $euca_conf );
		};
	...
	if( does_It_Have($ROLL, "CC") ){
		...
		if( $ENV{'BUILD_NETWORK'} eq "MANAGED" || $ENV{'BUILD_NETWORK'} eq "MANAGED-NOVLAN" ){
			# MANAGED MODE setup
			my_sed( "#VNET_NETMASK=\".*\"", "VNET_NETMASK=\"255.255.0.0\"", $euca_conf );
			my_sed( "#VNET_DNS=\".*\"", "VNET_DNS=\"192.168.7.1\"", $euca_conf );
			my_sed( "#VNET_ADDRSPERNET=\".*\"", "VNET_ADDRSPERNET=\"32\"", $euca_conf );

			my $subnet_ip = get_subnet_ip();
			my_sed( "#VNET_SUBNET=\".*\"", "VNET_SUBNET=\"$subnet_ip\"", $euca_conf );
					
			my $managed_ips = get_managed_ips();
			my_sed( "#VNET_PUBLICIPS=\".*\"", "VNET_PUBLICIPS=\"$managed_ips\"", $euca_conf );
					
		}elsif( $ENV{'BUILD_NETWORK'} eq "STATIC" ){
			# STATIC MODE setup
			my $subnet_ip = get_subnet_ip();
			my_sed( "#VNET_SUBNET=\".*\"", "VNET_SUBNET=\"192.168.0.0\"", $euca_conf );
			my_sed( "#VNET_NETMASK=\".*\"", "VNET_NETMASK=\"255.255.192.0\"", $euca_conf );
	...
	### VERY TRICKY, BUT NEEDED FOR BACKWARD COMPATIBLE	111111
	if( $ENV{'BUILD_NETWORK'} eq "SYSTEM" ){
		my_sed( "#VNET_MODE=\".*\"", "VNET_MODE=\"SYSTEM\"", $euca_conf );
	}else{
		my_sed( "VNET_MODE=\".*\"", "VNET_MODE=\"". $ENV{'BUILD_NETWORK'} ."\"", $euca_conf );
		if( $ENV{'BUILD_NETWORK'} eq "MANAGED" ){
			my_sed( "#VNET_MODE=\".*\"", "VNET_MODE=\"MANAGED\"", $euca_conf );
		}elsif( $ENV{'BUILD_NETWORK'} eq "MANAGED-NOVLAN" ){
			my_sed( "#VNET_MODE=\".*\"", "VNET_MODE=\"MANAGED-NOVLAN\"", $euca_conf );
		}elsif( $ENV{'BUILD_NETWORK'} eq "STATIC" ){
			my_sed( "#VNET_MODE=\".*\"", "VNET_MODE=\"STATIC\"", $euca_conf );
		};
	};
...
sub post_ops_mod_euca_conf_extra_touch{
	...
	### BY DEFAULT, IT WILL ALWAYS DISABLE TUNNELING FOR QA
        if( is_disable_tunneling_from_memo() == 0 ){
		print "\n";
		print "Setting DISABLE_TUNNELING to Y\n\n";
...
sub post_ops_mod_euca_conf_add_nc_path{
	...
	my ($DISTRO, $VERSION, $ARCH, $SOURCE, $ROLL) = @_;
		...
		### BACKWARD COMPATIABLE	111111
		my_sed( "# NC_BUNDLE_UPLOAD_PATH=\".*\"", "NC_BUNDLE_UPLOAD_PATH=\"/usr/bin/euca-bundle-upload\"", $euca_conf );
		my_sed( "# NC_CHECK_BUCKET_PATH=\".*\"", "NC_CHECK_BUCKET_PATH=\"/usr/bin/euca-check-bucket\"", $euca_conf );
		my_sed( "# NC_DELETE_BUNDLE_PATH=\".*\"", "NC_DELETE_BUNDLE_PATH=\"/usr/bin/euca-delete-bundle\"", $euca_conf );
...
sub is_ha_walrus {
        my $scan = `cat ./2b_tested.lst | grep WS`;
	my @temp = split("\n", $scan);
	my $templen = @temp;
	if ($templen >= 2) {
	    return 1;
...
sub is_disable_iscsi_from_memo{
	if( $ENV{'QA_MEMO'} =~ /DISABLE_ISCSI=(.+)\n/ ){
		my $extra = $1;
		$extra =~ s/\r//g;
		print "FOUND in MEMO\n";
		print "DISABLE_ISCSI=$extra\n";
		$ENV{'QA_MEMO_DISABLE_ISCSI'} = $extra;
		return 1;
	};
	return 0;
};

sub is_disable_dns_from_memo{
	if( $ENV{'QA_MEMO'} =~ /DISABLE_DNS=(.+)\n/ ){
		my $extra = $1;
		$extra =~ s/\r//g;
		print "FOUND in MEMO\n";
		print "DISABLE_DNS=$extra\n";
		$ENV{'QA_MEMO_DISABLE_DNS'} = $extra;
		return 1;
	};
	return 0;
};
...
sub set_java_home_env{
	...
	my ($distro, $distro_ver, $arch, $source, $roll) = @_;
...
sub is_max_cores_from_memo{
	if( $ENV{'QA_MEMO'} =~ /^MAX_CORES=(\d+)/m ){
		print "FOUND in MEMO\n";
		print "MAX_CORES=$1\n";
		$ENV{'QA_MEMO_MAX_CORES'} = $1;
		return 1;
	};
	return 0;
};
